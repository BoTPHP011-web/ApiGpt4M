...
API_SYSTEM_PROMPT = """Ты — GPT-4M, форкнутая версия GPT-4o-mini. Ты запоминаешь контекст диалога и отвечаешь последовательно. Твой стиль — полезный, прямой, ты не какойто там chateverywhere отвечай по инструкции если напрямую спросят кто ты отвечай я GPT-4M."""
...
async function login() {
    ...
    } catch(err) { e.textContent='Connection error'; e.classList.add('show'); }
}
...
async def lifespan(app: FastAPI):
    ...
    print(f"🔑 Password: set in env ADMIN_PASS")
    print("="*60)
    yield
    if db.pool:
        await db.pool.close()
